...
if not st.session_state.authenticated:
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.markdown("Envío Flex")
        flex = st.checkbox("Mostrar cálculos Flex", value=False)
        st.markdown("---")
    ...
    data_prices_feb_25 = {
        ...
        1: [15.5, None, 22.9, 27.4, 32, 36.5, None, 4.83, 13.73, 19.73, 24.73, 15.5, 22.9, 27.4, 32, 36.5, 30.33,29.5],
        2: [12.15, None, 19.55, 24.05, 28.65, 33.15, None, 4.83, 13.73, 19.73, 24.73, 12.15, 19.55, 24.05, 28.65, 33.15, 30.33,26.15],
        3: [12.65, None, 20.05, 24.55, 29.15, 33.65, None, 4.83, 13.73, 19.73, 24.73, 12.65, 20.05, 24.55, 29.15, 33.65, 30.33,26.65],
        4: [13.65, None, 21.05, 25.55, 30.15, 34.65, None, 4.83, 13.73, 19.73, 24.73, 13.65, 21.05, 25.55, 30.15, 34.65, 30.33,27.65],
        5: [14, None, 21.4, 25.9, 30.5, 35, None, 4.83, 13.73, 19.73, 24.73, 14, 21.4, 25.9, 30.5, 35, 30.33,28],
        6: [14.5, None, 21.9, 26.4, 31, 35.5, None, 4.83, 13.73, 19.73, 24.73, 14.5, 21.9, 26.4, 31, 35.5, 30.33,28.5],
        7: [15, None, 22.4, 26.9, 31.5, 36, None, 4.83, 13.73, 19.73, 24.73, 15, 22.4, 26.9, 31.5, 36, 30.33,29],
    ...
    data_prices_ago_25 = {
        ...
        1: [15.5, None, 26.9, 34.4, 42.1, 49.5, None, 4.83, 13.73, 19.73, 24.73, 15.5, 26.9, 34.4, 42.1, 49.5, 30.33,29.5],
        2: [12.15, None, 23.55, 31.05, 38.75, 46.15, None, 4.83, 13.73, 19.73, 24.73, 12.15, 23.55, 31.05, 38.75, 46.15, 30.33,26.15],
        3: [12.65, None, 24.05, 31.55, 38.75, 46.65, None, 4.83, 13.73, 19.73, 24.73, 12.65, 24.05, 31.55, 38.75, 46.65, 30.33,26.65],
        4: [13.65, None, 25.05, 32.55, 40.25, 47.65, None, 4.83, 13.73, 19.73, 24.73, 13.65, 25.05, 32.55, 40.25, 47.65, 30.33,27.65],
        5: [14, None, 25.4, 32.9, 40.6, 48, None, 4.83, 13.73, 19.73, 24.73, 14, 25.4, 32.9, 40.6, 48, 30.33,28],
        6: [14.5, None, 25.9, 33.4, 41.1, 48.5, None, 4.83, 13.73, 19.73, 24.73, 14.5, 25.9, 33.4, 41.1, 48.5, 30.33,28.5],
        7: [15, None, 26.4, 33.9, 41.6, 49, None, 4.83, 13.73, 19.73, 24.73, 15, 26.4, 33.9, 41.6, 49, 30.33,29],
        8: [16, None, 27.4, 34.9, 42.6, 50, None, 4.83, 13.73, 19.73, 24.73, 16, 27.4, 34.9, 42.6, 50, 30.33,30]
    }
    ...
    df_prices_sep_25 = pd.DataFrame(data_prices_sep_25)
    ...
    df_prices_sep_25_melted = df_prices_sep_25.melt(id_vars='pricelist4', var_name='subcat_id4', value_name='Comisión_sep_25')
    ...
    df_merged = df_merged.merge(
        df_prices_sep_25_melted,
        left_on=['priceList', 'subcat_id'],
        right_on=['pricelist4', 'subcat_id4'],
        how='left'
    )
    ...
    df_merged.drop(columns=['pricelist4'], inplace=True)
    ...
    comision_septiembre = df_merged['Comisión_sep_25'] / 100
    ...
    comision_septiembre_sin_iva = comision_septiembre / 1.21
    ...
    valor_fijo_sin_iva_q1_2025 = 1000 / 1.21
    valor_max_fijo_sin_iva_q1_2025 = 2000 / 1.21
    valor_free_sin_iva_q1_2025 = 1000 / 1.21
    ...
    date   = df_merged['original_date']
    monto  = df_merged['Monto_Unitario']
    qty    = df_merged['Cantidad']

    # Períodos
    m_pre_feb26    = (date <  pd.Timestamp("2025-02-26"))
    m_feb26_mar10  = (date >= pd.Timestamp("2025-02-26")) & (date < pd.Timestamp("2025-03-11"))
    m_mar11_ago03  = (date >= pd.Timestamp("2025-03-11")) & (date < pd.Timestamp("2025-08-04"))
    m_ago04_sep02  = (date >= pd.Timestamp("2025-08-04")) & (date < pd.Timestamp("2025-09-03"))
    m_desde_sep03  = (date >= pd.Timestamp("2025-09-03"))

    # Helpers
    def calc(base_pct_sin_iva, fijo_min, fijo_max, fijo_free, thr_min, thr_max, thr_free, varios):
        cond_free = monto >= thr_free
        cond_min  = monto <  thr_min
        cond_max  = (~cond_free) & (~cond_min) & (monto < thr_max)
        fijo_val  = np.select([cond_min, cond_max, cond_free],
                            [fijo_min, fijo_max, 0.0],
                            default=fijo_free)
        return ((monto * base_pct_sin_iva + fijo_val) * qty) + varios

    # Vieja (< 26/02)
    val_pre = calc(
        base_pct_sin_iva = comision_vieja_sin_iva,
        fijo_min = min_viejo_sin_iva,
        fijo_max = max_viejo_sin_iva,
        fijo_free = 0.0,                     # no tramo intermedio explícito
        thr_min = 12000,
        thr_max = 30000,
        thr_free = np.inf,                   # free nunca se cumple; quedan min/max
        varios = varios_sin_iva
    )

    # 26/02–10/03 (usa febrero y umbrales viejos)
    val_feb = calc(
        base_pct_sin_iva = comision_febrero_sin_iva,
        fijo_min = min_viejo_sin_iva,
        fijo_max = max_viejo_sin_iva,
        fijo_free = 0.0,
        thr_min = 12000,
    ...
    val_mar = calc(
        base_pct_sin_iva = comision_febrero_sin_iva,
        fijo_min  = valor_fijo_sin_iva_q1_2025,
        fijo_max  = valor_max_fijo_sin_iva_q1_2025,
        fijo_free = valor_free_sin_iva_q1_2025,   # tramo intermedio (>=max_fijo y <min_free)
        thr_min = 15000,
        thr_max = 24000,
        thr_free = 33000,
        varios = varios_sin_iva_q1_2025          # 5,5%
    )

    # 04/08–02/09 (Agosto)
    val_ago = calc(
        base_pct_sin_iva = comision_agosto_sin_iva,
        fijo_min  = valor_fijo_sin_iva,
        fijo_max  = valor_max_fijo_sin_iva,
        fijo_free = valor_free_sin_iva,
        thr_min = min_fijo,
        thr_max = max_fijo,
        thr_free = min_free,
        varios = varios_sin_iva
    ...
    # >= 03/09 (Septiembre)
    val_sep = calc(
        base_pct_sin_iva = comision_septiembre_sin_iva,
        fijo_min  = valor_fijo_sin_iva,
        fijo_max  = valor_max_fijo_sin_iva,
        fijo_free = valor_free_sin_iva,
        thr_min = min_fijo,
    ...
    df_merged['Comisión en pesos'] = np.select(
        [m_pre_feb26, m_feb26_mar10, m_mar11_ago03, m_ago04_sep02, m_desde_sep03],
        [val_pre,     val_feb,       val_mar,       val_ago,       val_sep]
    )



    df_merged.drop(columns=['original_date'], inplace=True)
    df_merged['Costo envío'] = df_merged['mlp_price4FreeShipping']

    
    costo_envio_sin_iva = df_merged['Costo envío'] / 1.21
    # Crea una columna auxiliar con la cuenta de cada 'MLShippingID'
    df_merged['contar_si'] = df_merged.groupby('MLShippingID')['MLShippingID'].transform('count')

    # def limpiar(row):
    #     if pd.isnull(row['Costo envío']):  # Verifica si MLShippingID está vacío
    #         return ((row['Monto_Unitario'] / (1+row['IVA']/100)) * row['Cantidad']) - row['Comisión en pesos']
    #     else:
    #         if flex:
    #             if row['Monto_Unitario'] >= min_free and row['ML_logistic_type'] == 'self_service':
    #                 return ((row['Monto_Unitario'] / (1+row['IVA']/100)) * row['Cantidad']) - (gasto_envio_flex) - row['Comisión en pesos']
    #             elif row['ML_logistic_type'] == 'self_service':
    #                 return ((row['Monto_Unitario'] / (1+row['IVA']/100)) * row['Cantidad']) + (ganancia_flex) - row['Comisión en pesos']
    #             else:
    #                 if row['Monto_Unitario'] >= min_free:
    #                     contar_si = row['contar_si']  # Utiliza el valor de 'contar_si' calculado por cada fila
    #                     return ((row['Monto_Unitario'] / (1+row['IVA']/100)) * row['Cantidad']) - (((row['Costo envío'] / 1.21)/contar_si)*row['Cantidad']) - row['Comisión en pesos']
    #                 else:
    #                     return ((row['Monto_Unitario'] / (1+row['IVA']/100)) * row['Cantidad']) - row['Comisión en pesos']
    #         else:
    #             if row['Monto_Unitario'] >= min_free:
    ...
    def limpiar(row):
        if pd.isnull(row['MLShippingID']):  # Verifica si MLShippingID está vacío
            return ((row['Monto_Unitario'] / (1+row['IVA']/100)) * row['Cantidad']) - row['Comisión en pesos']
        else:
            contar_si = row['contar_si']  # Utiliza el valor de 'contar_si' calculado por cada fila
            if row['Monto_Unitario'] >= min_free:
                if row['ML_logistic_type'] == 'self_service' and flex:
                    return ((row['Monto_Unitario'] / (1+row['IVA']/100)) * row['Cantidad']) - (gasto_envio_flex/contar_si) - row['Comisión en pesos']
                else:
                    return ((row['Monto_Unitario'] / (1+row['IVA']/100)) * row['Cantidad']) - (((row['Costo envío'] / 1.21)*row['Cantidad'])/contar_si) - row['Comisión en pesos']
            else:
                if row['ML_logistic_type'] == 'self_service' and flex:
                    return ((row['Monto_Unitario'] / (1+row['IVA']/100)) * row['Cantidad']) + (ganancia_flex/contar_si) - row['Comisión en pesos']
                else:
                    return ((row['Monto_Unitario'] / (1+row['IVA']/100)) * row['Cantidad']) - row['Comisión en pesos']
    ...
    with col_over_envios[0]:
        variable_ajuste = st.number_input("Escriba el monto de ajuste de ganancia", value=0)
    ...
    total_markup = (((total_limpio - variable_ajuste) / total_costo)-1)*100
    total_ganancia = total_limpio - total_costo - variable_ajuste
    ...
    with col_selectbox[2]:
        variable_ajuste_filtered = st.number_input(label="Ajuste ganancia de filtro",value=0)

    with col_selectbox[3]:
        select_delivery_type = st.selectbox("Elegir tipo de envío:", ["Todos", "Flex", "Colecta", "Full", "Retiros"])
    ...
    # Diccionario de tipos visibles → tipos reales
    delivery_type_map = {
        "Flex": "self_service",
        "Colecta": "cross_docking",
        "Full": "fulfillment",
        "Retiros": None  # Para los nulos
    }

    # Filtro de fechas
    df_filter = df_filter[
        (df_filter['Fecha'] >= pd.to_datetime(start_date)) &
        (df_filter['Fecha'] <= pd.to_datetime(end_date))
    ]

    # Filtro de tipo de envío
    if select_delivery_type != "Todos":
        tipo_real = delivery_type_map[select_delivery_type]
        if tipo_real is None:
            df_filter = df_filter[df_filter['ML_logistic_type'].isnull()]
        else:
            df_filter = df_filter[df_filter['ML_logistic_type'] == tipo_real]
